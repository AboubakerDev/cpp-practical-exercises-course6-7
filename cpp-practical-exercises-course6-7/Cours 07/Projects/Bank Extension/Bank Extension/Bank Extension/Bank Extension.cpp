...
void PrintClientRecord(sClient Client, bool b = true)
{
    if (b)
    {
...
void AddClients() {
    ...
    do {
        ...
        SaveStringToFile(ClientsFileName, S);
...
void PressAnyKey(string s="main menu")
{
    cout << "\n\n\nPress any key to go back to "<< s <<"...";
    _getch();
}
...
void DepositAmount(string AccountNumber)
{
    float DepositAmount,NewBalance ;
    string Answer = "N";
    ...
    if (Answer == "y" || Answer == "Y")
    {
        ...
        SaveCleintsDataToFile(ClientsFileName, vClients);
        cout << "\n\nDon Successfully New Balance is: " << NewBalance;
    }
}

void Deposit()
{
    ScreenTitle("Deposit Screen");
    string AccountNumber;
...
{
    string text = "Transaction Menu Screen";
    int width = 60;
...
void TotalBalances(vector <sClient> vClients)
{
    ...
    for (sClient Client : vClients)
    {
        PrintClientRecord(Client,false);
...
{
    float WithdrawAmount,NewBalance;
    sClient Client;
    string Answer = "N";
...
void Transaction()
{
    ...
    case  TOTAL_BALANCES:
        system("cls");
        TotalBalances(LoadCleintsDataFromFile(ClientsFileName));
        PressAnyKey("transaction menu");
        Transaction();
        break;
    case MAINMENUE:
